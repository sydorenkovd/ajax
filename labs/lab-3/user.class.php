...
class User{
	...
	public function __construct(){
		...
		if (! @$this->db->querySingle('SELECT COUNT(*) + 1 FROM user')){
			$this->db->query('CREATE TABLE user
				(
					id INTEGER PRIMARY KEY AUTOINCREMENT, -- Код записи пользователя
		...
		if (! @$this->db->querySingle('SELECT COUNT(*) + 1 FROM session')){
	...
	public function add($userInfo){
		$name = $this->db->escapeString($userInfo->name);
		$login = $this->db->escapeString(strtolower($userInfo->login));
		$hash = md5($login . $userInfo->password);
		// Если такого пользователя нет...
		if (! $this->db->querySingle("SELECT COUNT(id) FROM user WHERE login='$login'"))
	...
	public function validate($userInfo){
		...
		$userId = $this->db->querySingle("SELECT id FROM user 
